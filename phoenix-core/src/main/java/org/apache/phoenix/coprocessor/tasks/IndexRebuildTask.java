    ...
    public TaskRegionObserver.TaskResult run(Task.TaskRecord taskRecord) {
        ...
        try {
            ...
            if (jsonObject.has(REBUILD_ALL)) {
                String rebuildAll = jsonObject.get(REBUILD_ALL).toString();
                if (!Strings.isNullOrEmpty(rebuildAll)) {
                    deleteBeforeRebuildAll = Boolean.valueOf(rebuildAll);
                }
            }
            ...
            Map.Entry<Integer, Job> indexToolRes = IndexTool
                    .run(conf, taskRecord.getSchemaName(), taskRecord.getTableName(), indexName, true,
                            false, taskRecord.getTenantId(), shouldDisable, deleteBeforeRebuildAll, runForeground);
